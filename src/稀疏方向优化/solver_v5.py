...
def _scarce_mix_weights(meta):
    if meta.get("courier_count", 0) / max(meta.get("task_count", 1), 1) < 1.25:
        return {"single": 0.25, "pair": 0.65, "triple": 0.1}
    return {"single": 0.0, "pair": 1.0, "triple": 0.0}
    ...
    if low_case:
        ...
        if scarce_case and meta.get("task_count", 0) <= 44 and meta.get("max_bundle_size", 0) <= 2:
            for selected in _scarce_pair_matching_candidates(candidates, meta):
                experiments.append(selected)
        if scarce_case and meta.get("max_bundle_size", 0) >= 3:
            for selected in _scarce_triple_candidates(candidates, meta):
                experiments.append(selected)
...
def _scarce_pair_matching_candidates(candidates, meta):
    ...
    reject_penalty = _reject_penalty(meta)
    mix_weights = _scarce_mix_weights(meta)
    scarce_pair_ratio = mix_weights["pair"]
    single_cost = {}
...
def _scarce_triple_candidates(candidates, meta):
    triple_rows = [item for item in candidates if len(item[0]) == 3]
    if not triple_rows:
        return []

    mix_weights = _scarce_mix_weights(meta)
    if mix_weights["triple"] <= 0.0:
        return []
    ...
    for item in triple_rows:
        ...
        preview = rows[:4]
        expected = _expected_bundle_score(preview, reject_penalty)
        weighted_cost = expected / 3 - 120.0 * mix_weights["triple"]
        triple_infos.append((weighted_cost, expected, task_set, rows))

    for sorter in (
        lambda item: (item[0], item[1], tuple(sorted(item[2]))),
        lambda item: (item[1], item[0], tuple(sorted(item[2]))),
    ):
        used_tasks = set()
        used_couriers = set()
        selected = []
        for _weighted_cost, _expected, task_set, rows in sorted(triple_infos, key=sorter):
            if task_set & used_tasks:
                continue
            row = next((candidate for candidate in rows[:100] if candidate[2] not in used_couriers), None)
            if row is None:
                continue
            selected.append(row)
            used_tasks.update(task_set)
            used_couriers.add(row[2])
        if selected:
            results.append(selected)
    return results
